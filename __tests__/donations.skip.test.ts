...
// const { describe } = require('../validators/eventSchema');

// let adminToken, managerToken, manager2Token, user1Token, user2Token, createdDonationId;
...
//   test('Filter by status=active (single donation)', async () => {
//     const res = await request(app)
//       .get(`/api/donations/${createdDonationId}`)
//       .set('Authorization', `Bearer ${user1Token}`);

//     expect(res.statusCode).toBe(200);
//     expect(res.body.donation.status).toBe('active');
//   });
...
//       expect(Number(donation.total_collected)).toBe(0);
//     });
//   });

//   test('donor_count increases after donation', async () => {
//     // Get current donor_count
//     const resBefore = await request(app)
//       .get(`/api/donations/${createdDonationId}`)
//       .set('Authorization', `Bearer ${user1Token}`);

//     const oldCount = resBefore.body.donation.donor_count;

//     // Make a donation
//     await request(app)
//       .post(`/api/donations/${createdDonationId}/confirm-payment`)
//       .set('Authorization', `Bearer ${user1Token}`)
//       .send({ amount: 2 });

//     // Get updated donor_count
//     const resAfter = await request(app)
//       .get(`/api/donations/${createdDonationId}`)
//       .set('Authorization', `Bearer ${user1Token}`);

//     const newCount = resAfter.body.donation.donor_count;
...
//         description: 'Testing applicants endpoint',
//         deadline: '2099-12-31'
//       });

//     donationId = create.body.donation.id;
//   });
